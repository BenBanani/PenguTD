...
public class TowerAnimator {
    ...
    public TowerAnimator(@NotNull String name, @NotNull TextureAtlas atlas) {
        ...
        attackStart = Assets.findRegionOrMissing(atlas, name + "_attack1");
    ...
    public @NotNull TextureRegion getTexture(float timeSinceLastAttack, float timeToNextAttack, float totalCooldown, boolean isAttacking) {
        if (!isAttacking) return idle;
        if (timeToNextAttack < 0.1 * totalCooldown) return attackStart;
        if (timeSinceLastAttack < 0.1 * totalCooldown) return attackShoot;
        if (timeSinceLastAttack < 0.7 * totalCooldown) return attackShotOver;
        return idle;
    }
}
